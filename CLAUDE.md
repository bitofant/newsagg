...
3. **User DB** (`src/db/users.ts`) - User management + preferences + signal queue + read tracking (SQLite)
   ...
   - Two preference fields per user:
...
5. **Consolidator** (`src/consolidator/`) - Topic matching, signal generation & summary generation
   ...
   - **Topic unmerging**: `unmergeTopic(topicId)` splits a wrongly-merged topic via a two-phase LLM call: **phase 1** (reasoning, one call) reads all article titles + the original topic and outputs sub-topic descriptors (defaulting to 2 — see prompt; the typical wrong-merge is "one big topic + a small off-topic group that snuck in", so unbalanced 2-way splits are the expected output, not a fan-out into 5+ buckets); **phase 2** (classification, no reasoning, parallel via `Promise.all`) chunks the articles by token budget and assigns each to one of the phase-1 sub-topics by index. New topics are created and their summaries regenerated, read state is copied from the old topic to all new topics (per-user via `db.users.replaceReadTopic`), and every user's latest front page that referenced the old topic is rewritten in place — the section is spliced out and the new sections are spliced in at the same index, then saved as a new `front_pages` row so SSE pushes the update. The old topic is deleted last. Returns `{ newTopicIds, affectedUserIds }`. The HTTP layer wraps this asynchronously: `POST /api/topics/:id/unmerge` kicks off the work and returns immediately; `GET /api/topics/:id/unmerge-result?wait=N` long-polls (up to 60s, server-clamped) for completion and returns `{ status: 'pending' | 'done' | 'error', newTopics?, error? }`. Job state lives in an in-memory `Map<topicId, UnmergeJob>` in the server with a 5-minute TTL.
...
8. **UI** (`ui/`) - SvelteKit + Tailwind CSS
   ...
   - Per-topic unmerge button (`Split` icon) on both the front-page card and the detail page: opens an absolutely-positioned overlay with four phases — `confirm` (split-pane: 60%-opacity green thumbs-up to commit / red thumbs-down to cancel, with "Split topic?" headline), `pending` (yellow background + spinning `Loader2`), `done` (green background listing the new topic titles, tap-to-dismiss → refreshes the front page or routes back to `/`), `error` (red background with the error message). The flow uses long-polling: POST kicks off, then `pollUnmergeResult` repeatedly with `wait=30`. While any overlay is in pending/done/error state, SSE-driven page replacements are held in `pendingFreshPage` and applied on dismiss so the card the user is looking at doesn't disappear under the overlay. The card column is getting crowded — flagged with a TODO for a future overflow-menu redesign.
...
- ~~**User preferences expansion**~~ → **IMPLEMENTED** — `interval_ms` + `manual_preferences` (user-edited) + `preference_profile` (generated, read-only) exposed via `GET/PATCH /api/preferences` and `/settings` UI; PATCH only accepts `manualPreferences`, sending `preferenceProfile` returns 400. Profile auto-generated from manual input + votes via profiler.
