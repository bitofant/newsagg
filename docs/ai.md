...
### vLLM auto-detection via `"auto"` sentinel (2026-04-25, gotcha noted 2026-05-03)
`config.ai.model` and `config.ai.maxContextTokens` accept `"auto"` to fetch the first model's `id` and `max_model_len` from `GET /v1/models` at startup. `VllmProvider.doInit()` resolves these before the first `complete()` call. One fetch covers both fields. vLLM always includes `max_model_len`; Ollama does not — setting `"auto"` with Ollama backend throws a clear error. Rationale: vLLM typically serves a single model and the context window is a property of the model weights, not something operators should have to look up manually.

**Gotcha — `maxContextTokens === 0` until init runs.** The `InferenceProvider` constructor leaves `resolvedMaxContextTokens = 0` whenever the config value is `"auto"`; only `doInit()` (lazy, fired by the first `complete()`) writes the real value. Any code that *reads* `ai.maxContextTokens` *before* `complete()` has run sees 0 and silently misbehaves. The consolidator's `inputBudget()` is the canonical victim: with `ai.maxContextTokens === 0`, the formula `maxContextTokens − outputReserve − overhead − margin` goes deeply negative and the `MIN_INPUT_BUDGET` floor (512) wins, so `chunkByTokens` produces tiny ~14-topic chunks instead of one 50-topic chunk on the first batch after process startup. That broke vLLM prefix caching: the first batch seeded the cache with sub-1568-token chunks (no full block could form), and later batches with proper 50-topic chunks had nothing to hit against. Mitigation: every consolidator entry point that reaches a chunked function calls `await ai.ensureInitialized()` immediately after `getAi()` (drain main path, drain regen path, `ungroupArticle`, `unmergeTopic`). Add the same await at any new entry point that performs chunking before the first `complete()`.
...
Within the user prompt, place longer-stable content first — and pick the axis whose stable section is large enough to actually fill one cache block. vLLM's block size is normally 16 tokens, but **hybrid attention/Mamba models** (e.g. `Qwen3_5ForConditionalGeneration` → block size 1568) force much larger blocks: a stable prefix shorter than one block produces zero hits. So size matters more than nominal stability. Two examples:

- `matchBatchAgainstTopics` puts the per-chunk **topic page** before the article batch. `processBatch` re-runs every article batch in a drain against the same paginated topic pages (deterministic DB pagination), so a rendered topic page is byte-identical across batches. At ~50 topics ≈ 1900 tokens it fills a 1568 block; the article batch (~150 tokens for 2 articles) never could. The intra-batch sibling-sharing on articles is sacrificed because that prefix was too small to matter on hybrid models.
- The aggregator's relevance scorer puts the per-user preference profile before the per-tick topic list so consecutive front-page generations for the same user share the profile prefix.
