...
7. **Profiler** (`src/profiler/`) - User preference profile generation
   ...
   - When timer fires: reads `users.manual_preferences` + vote history with article/topic context, asks LLM to write a unified profile that preserves the manual preferences verbatim and augments them with inferred interests from votes
